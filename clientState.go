...
func newClientState(l lane.Lane, client RedisClient, dispatcher *cmdDispatcher) *clientState {
	cs := &clientState{
		l:           l,
		user:        "default",
		client:      client,
...
func (cs *clientState) unregister() {
	...
	if info.connected_clients != int64(len(clients)) {
		panic("statistics out of sync with client table")
	}
	infoMu.Unlock()
}
...
func (cs *clientState) dispatch(input respValue) (output respValue) {
	return cs.disp.dispatch(cs, input)
}
...
func (cs *clientState) selectDb(index int, create bool) (priorSelection int, valid bool) {
	cs.mu.Lock()
	defer cs.mu.Unlock()

	priorSelection = cs.selectedDb
	ds, valid := cs.dss.getDb(index, true)
	if !valid {
		return
	}
